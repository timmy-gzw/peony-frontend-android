...
//    private ImageView ivBackground;
    ...
    private long mTimeRefresh;//家族频繁刷新item
...
//                    container.proxy.shouldCollapseInputPanel();
        ...
        // adapter
        items = new ArrayList<>();
        adapter = new MsgAdapter(messageListView, linearLayoutManager, items, container);
    ...
    public void removeMessageItem(String uuid) {
        if (TextUtils.isEmpty(uuid)) {
            return;
        }
        IMMessage msg;
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (uuid.equals(msg.getUuid()) && items.size() > i) {
                removeViewHolderByIndex(i);
                break;
            }
        }
    }
     ...
     * 删除单条消息
     */
    private void removeViewHolderByIndex(final int index) {
        try {
            uiHandler.post(new Runnable() {
                @Override
                public void run() {
...
//        if (adapter.getBottomDataPosition() > 0)
//            messageListView.smoothScrollToPosition(adapter.getBottomDataPosition());
    }


    Handler handler = new Handler(Looper.getMainLooper()) {//解决家族 发送频繁显示重复问题
        @Override
...
//            adapter.notifyDataSetChanged();
    ...
    public void onIncomingMessage(List<IMMessage> messages) {
        ...
        for (IMMessage message : messages) {
            ...
            ChatMsgUtil.removeMessage(items, message, String.valueOf(service.getUserId()));
        }
        if (needRefresh) {
            if (container.sessionType == SessionTypeEnum.Team) {
                if (System.currentTimeMillis() - mTimeRefresh > 500) {
                    mTimeRefresh = System.currentTimeMillis();
                    adapter.notifyItemRangeInserted(items.size() - addedListItems.size() + adapter.getFetchMoreViewCount(), addedListItems.size());
                    sortMessages(items);
                } else {
                    mTimeRefresh = System.currentTimeMillis();
                    handler.removeCallbacksAndMessages(null);
...
//                incomingMsgPrompt.show(lastMsg);   //隐藏
            }
        }
    }
    ...
    public void onMsgSend(IMMessage message) {
        ...
        if (null != chatMsg && (TextUtils.equals(chatMsg.cmd_type, ChatMsg.CALL_TYPE))) {
        ...
        public void onEvent(IMMessage message) {
            ...
            try {
                if (null != chatMsg && TextUtils.equals(chatMsg.cmd_type, ChatMsg.CALL_TYPE)) { //语音视频消息回调
                    ...
                    if (callMsg1.state == 0) {
                        ...
                        VideoCallActivity.startCallOther(BaseApplication.getInstance(), userInfo, chatMsg.type, callMsg1.call_id, callMsg1.type, callMsg1.to_user_is_online, isMatch);
            ...
            if (isMyMessage(message)) {
                ...
                if (null != messageListener) {
                    ...
                    messageListener.getCallBackContent(callBackMessage, message, message.getCallbackExtension());
    ...
    public interface OnMessageListener {
        void getCallBackContent(ChatMsg.CallBackMessage callBackMessage, IMMessage message, String content);
    }
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    public void replaceItem(String uuid, IMMessage msg) {
        int itemIndex = getItemIndex(uuid);
        if (itemIndex != -1) {
            items.set(itemIndex, msg);
            adapter.notifyDataItemChanged(itemIndex);
        }
    }
...
//            ivBackground.setBackgroundColor(color);
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private void loadFromRemote() {
            ...
            NIMClient.getService(MsgService.class)
                    // 0:文本，1:图片，2:语音，3:视频，4:地理位置，5:通知，6:文件，10:提示，100:自定义
//            (anchor, 0, limit, QueryDirectionEnum.QUERY_OLD, persist, null, persistClear, null)
                    .pullMessageHistoryExType(anchor(), 0, 80, QueryDirectionEnum.QUERY_OLD,
        ...
        private void onMessageLoaded(final List<IMMessage> messages) {
            ...
            ChatMsgUtil.removeMessage(messages);
...
//            }
    ...
    private class MsgItemEventListener extends MsgAdapter.BaseViewHolderEventListener {
        ...
        private void reSendMessage(IMMessage message) {
            CustomPopWindow popWindow = new CustomPopWindow(container.activity);
            popWindow.setContent("是否确认重新发送该消息");
        ...
        public void onFailedBtnClick(IMMessage message) {
            if (message.getDirect() == MsgDirectionEnum.Out) {
                ...
                if (message.getStatus() == MsgStatusEnum.fail && !container.activity.isFinishing()) {
...
//            longClickItemReply(selectedItem, alertDialog, msgType);
    ...
    public void isInTeam(String sessionId, String account, RequestCallback<TeamMember> requestCallback) {
        InvocationFuture<TeamMember> invocationFuture = NIMClient.getService(TeamService.class).queryTeamMember(sessionId, account);
     ...
     * @param msgId uuid
     * @param msg   全称text
     * @param desc  设置颜色的text
     */
    public void adapterNotifyAward(String msgId, String msg, String desc) {

        if (adapter != null && adapter.getData() != null && adapter.getData().size() > 0) {
            for (int i = 0; i < adapter.getData().size(); i++) {
                if (adapter.getData().get(i).getUuid().equals(msgId)) {
                    Map<String, Object> map = adapter.getData().get(i).getLocalExtension();
                    if (map == null) {
                        map = new HashMap<>();
                    }
                    map.put("award", msg);
                    map.put("color", desc);
                    adapter.getData().get(i).setLocalExtension(map);
                    adapter.notifyDataItemChanged(i);
                    NIMClient.getService(MsgService.class).updateIMMessage(adapter.getData().get(i));
                    return;
                }
            }
        }
