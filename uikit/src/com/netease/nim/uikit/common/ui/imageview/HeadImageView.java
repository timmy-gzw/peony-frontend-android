...
public class HeadImageView extends CircleImageView {
    ...
    public void loadBuddyAvatar(String account) {
        ...
        if (userInfo != null && null != getConfig(context) && null != getConfig(context).api && null != getConfig(context).api.oss && null != getConfig(context).api.oss.cdn)
            url[0] = getConfig(context).api.oss.cdn_scheme + getConfig(context).api.oss.cdn.user + userInfo.getAvatar();
            ...
            NIMClient.getService(UserService.class).fetchUserInfo(list).setCallback(new RequestCallbackWrapper<List<NimUserInfo>>() {
                ...
                public void onResult(int code, List<NimUserInfo> result, Throwable exception) {
                    if (result != null && result.size() > 0) {
                        if (userInfo != null && null != getConfig(context) && null != getConfig(context).api && null != getConfig(context).api.oss && null != getConfig(context).api.oss.cdn)
                            url[0] = getConfig(context).api.oss.cdn_scheme + getConfig(context).api.oss.cdn.user + result.get(0).getAvatar();
                        changeUrlBeforeLoad(null, url[0],
...
//                    });
    ...
    private void loadImage(final String url, final int defaultResId, final int thumbSize) {
        RequestOptions requestOptions = new RequestOptions().placeholder(defaultResId)
                .error(defaultResId).diskCacheStrategy(DiskCacheStrategy.ALL);
