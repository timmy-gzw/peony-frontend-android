...
public class MoonUtil {
    ...
    public static void identifyFace(Context context,
                                    View textView, String content, List<String> account, String value, int align) {
        SpannableString mSpannableString = replaceEmoticons(context, value, DEF_SCALE, align);
        TeamMemberAitHelper.replaceAitBlueForeground(content, account, value, mSpannableString);
        viewSetText(textView, mSpannableString);
    }
    ...
    public static SpannableString makeSpannableStringTags(Context context, String value, TextView textView, int align) {
        if (TextUtils.isEmpty(value)) {
            value = "";
        }
        SpannableString mSpannableString = new SpannableString(value);
        ...
        while (matcher.find()) {
            ...
            int end = matcher.end();
            String emot = value.substring(start, end);
            Drawable d = getEmotDrawable2(context, emot, textView, DEF_SCALE);
            if (d != null) {
                ImageSpan span = new ImageSpan(d, align);
                mSpannableString.setSpan(span, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
    ...
    private static Drawable getEmotDrawable2(Context context, String text, TextView textView, float scale) {
        Drawable drawable = EmojiManager.getDrawable(context, text);
        // scale
        if (drawable != null) {
            int par = DensityUtils.dp2px(context, 13);
            drawable.setBounds(0, 0, par, par);
        }
        return drawable;
    }
