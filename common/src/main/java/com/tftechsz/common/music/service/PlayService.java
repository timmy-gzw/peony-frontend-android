package com.tftechsz.common.music.service;
...
import com.tftechsz.common.music.base.BaseConfig;
...
import com.tftechsz.common.music.manager.AudioFocusManager;
import com.tftechsz.common.music.manager.MediaSessionManager;
import com.tftechsz.common.music.receiver.AudioEarPhoneReceiver;
import com.tftechsz.common.music.util.QuitTimer;
...
/**
 * 包 名 : com.tftechsz.common.music.service
 * 描 述 : TODO
 */
...
//    private final AudioBroadcastReceiver mAudioReceiver = new AudioBroadcastReceiver();
    ...
    private final Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == UPDATE_PLAY_PROGRESS_SHOW) {
...
//        NotificationUtils.get().cancelAll();
        ...
        if(BaseMusicHelper.get()!=null&&BaseMusicHelper.get().getPartyService()!=null){
            BaseMusicHelper.get().getPartyService().leaveRoomService();
        }
    }
...
//            NotificationUtils.get().showPause(mPlayingMusic);
    ...
    public void seekTo(int progress) {
        ...
        if (isPlaying() || isPausing()) {
            ...
            if (mListener != null) {
                NERtcEx.getInstance().setAudioMixingPosition(progress);
     ...
     * @param music music
     */
    public void play(int pos, AudioBean music) {
        mPlayingMusic = music;
        createMediaPlayer();
        try {
            mPlayer.reset();
            //把音频路径传给播放器
            if (hasLocalFile(music.getPath())) {
                Utils.logE("文件存在， 本地播放");
                mPlayer.setDataSource(getLocalFile(music.getPath()));
                if (mListener != null) {
                    mListener.onBufferingUpdate((int) music.getDuration());
                }
            } else {
                Utils.logE("文件不存在， 下载播放");
                downloadMusic(mPlayingMusic.getPath());
                mPlayer.setDataSource(mPlayingMusic.getPath());
            }
//            mPlayer.setDataSource("http://public-cdn1.peony125.com/music/%E5%85%B6%E5%AE%9E%E6%88%91%E4%BB%AC%E6%B2%A1%E6%9C%89%E4%BB%80%E4%B9%88%E4%B8%8D%E5%90%8C-%E7%BD%91%E7%BB%9C.mp3");
            //准备
            ...
            //当播放的时候，需要刷新界面信息
            if (mListener != null) {
                mListener.onChange(pos, mPlayingMusic);
                if (hasLocalFile(music.getPath())) {
                    mListener.onBufferingUpdate((int) music.getDuration());
                }
            }
...
//            NotificationUtils.get().showPlay(mPlayingMusic);
    ...
    private void downloadMusic(String path) {
        DownloadHelper.download(path, DownloadHelper.FILE_PATH + File.separator + "music", null);
    }

    public boolean hasLocalFile(String fileName) {
        return FileUtils.isFileExists(getLocalFile(fileName));
    }
    ...
    public String getLocalFile(String fileName) {
        ...
        if (split.length > 0)
            return DownloadHelper.FILE_PATH + File.separator + "music" + File.separator + split[split.length - 1];
        return "";
    }
    ...
    private final MediaPlayer.OnBufferingUpdateListener mOnBufferingUpdateListener = new MediaPlayer.OnBufferingUpdateListener() {
        ...
        public void onBufferingUpdate(MediaPlayer mp, int percent) {
            if (mListener != null) {
                // 缓冲百分比
                mListener.onBufferingUpdate((int) (percent / (100d / getPlayingMusic().getDuration())));
            }
        }
    };
     ...
     * 跳转完成时的监听
     */
    private final MediaPlayer.OnSeekCompleteListener mOnSeekCompleteListener = mp -> {
     ...
     * 设置音频信息监听器
     */
    private final MediaPlayer.OnInfoListener mOnInfoListener = (mp, what, extra) -> false;
    ...
    public boolean isHaveNext() {
        if (audioMusics != null && audioMusics.size() > 0) {
            // 如果不是最后一首，则还有下一首
            // 如果是最后一首，则切换回第一首
            return mPlayingPosition != audioMusics.size() - 1;
        } else {
            return false;
        }
    }

    public void updateMusicList(List<AudioBean> list) {
        if (!list.isEmpty()) {
            audioMusics = list;
        } else {
    ...
    public void updateMusicList(final EventCallback<List<AudioBean>> callback) {
        Observable.create(new ObservableOnSubscribe<List<AudioBean>>() {
            @Override
            public void subscribe(@NonNull ObservableEmitter<List<AudioBean>> emitter) throws Exception {
                List<AudioBean> audioBeans = FileMusicScanManager.getInstance().scanMusic(PlayService.this);
                ...
                .subscribe(new Observer<List<AudioBean>>() {
                    ...
                    @Override
                    public void onNext(@NonNull List<AudioBean> audioBeans) {
                        BaseMusicHelper.get().setMusicList(audioBeans);
                        //如果获取音乐数据集合不为空
                        if (!BaseMusicHelper.get().getMusicList().isEmpty()) {
                            //音频的集合
                            audioMusics = BaseMusicHelper.get().getMusicList();
                            //刷新正在播放的本地歌曲的序号
                            updatePlayingPosition();
                            //获取正在播放的音乐
                            if (mPlayingPosition >= 0) {
                                mPlayingMusic = BaseMusicHelper.get().getMusicList().get(mPlayingPosition);
                            }
                        }
                        if (callback != null) {
                            callback.onEvent(audioBeans);
                        }
                    }
