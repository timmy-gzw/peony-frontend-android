package com.tftechsz.common.custom;
...
import com.tftechsz.common.utils.face.LimitFpsUtil;
...
public class CameraRenderer implements GLSurfaceView.Renderer, Camera.PreviewCallback {
    ...
    private final OnRendererStatusListener mOnRendererStatusListener;
    private int mViewWidth;
    private int mViewHeight;
    private Camera mCamera;
    private volatile boolean mIsStoppedPreview;
    ...
    private void openCamera(final int cameraFacing) {
        try {
            ...
            int[] size = CameraUtils.choosePreviewSize(parameters, DEFAULT_CAMERA_WIDTH, DEFAULT_CAMERA_HEIGHT);
            mCameraWidth = size[0];
...
//                            .show();
    ...
    private void startPreview() {
        ...
        try {
            ...
            if (mNv21ByteCopy == null) {
                mNv21ByteCopy = new byte[mCameraWidth * mCameraHeight * ImageFormat.getBitsPerPixel(ImageFormat.NV21) / 8];
            }
    ...
    private void destroyGlSurface() {
        ...
        if (mCameraTextureId > 0) {
            GlUtil.deleteTextureId(new int[]{mCameraTextureId});
            mCameraTextureId = 0;
        }
