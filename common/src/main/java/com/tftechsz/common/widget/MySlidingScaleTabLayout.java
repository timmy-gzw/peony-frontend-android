 ...
 * 描 述 : TODO
 */
public class MySlidingScaleTabLayout extends HorizontalScrollView {
    ...
    private MyTabScaleTransformer iTabScaleTransformer;

    private MyExtendTransformer extendTransformer;

    public MySlidingScaleTabLayout(Context context) {
        this(context, null, 0);
    }

    public MySlidingScaleTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MySlidingScaleTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MySlidingScaleTabLayout);

        mIndicatorStyle = ta.getInt(R.styleable.MySlidingScaleTabLayout_tl_indicator_style, STYLE_NORMAL);
        mIndicatorColor = ta.getColor(R.styleable.MySlidingScaleTabLayout_tl_indicator_color, Color.parseColor(mIndicatorStyle == STYLE_BLOCK ? "#4B6A87" : "#ffffff"));
        mIndicatorHeight = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_height,
                dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 4 : (mIndicatorStyle == STYLE_BLOCK ? -1 : 2)));
        mIndicatorWidth = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_width, dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 10 : -1));
        mIndicatorCornerRadius = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_corner_radius, dp2px(mIndicatorStyle == STYLE_BLOCK ? -1 : 0));
        mIndicatorMarginLeft = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_margin_left, dp2px(0));
        mIndicatorMarginTop = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_margin_top, dp2px(mIndicatorStyle == STYLE_BLOCK ? 7 : 0));
        mIndicatorMarginRight = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_margin_right, dp2px(0));
        mIndicatorMarginBottom = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_indicator_margin_bottom, dp2px(mIndicatorStyle == STYLE_BLOCK ? 7 : 0));
        mIndicatorGravity = ta.getInt(R.styleable.MySlidingScaleTabLayout_tl_indicator_gravity, Gravity.BOTTOM);
        mIndicatorWidthEqualTitle = ta.getBoolean(R.styleable.MySlidingScaleTabLayout_tl_indicator_width_equal_title, false);

        mUnderlineColor = ta.getColor(R.styleable.MySlidingScaleTabLayout_tl_underline_color, Color.parseColor("#ffffff"));
        mUnderlineHeight = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_underline_height, dp2px(0));
        mUnderlineGravity = ta.getInt(R.styleable.MySlidingScaleTabLayout_tl_underline_gravity, Gravity.BOTTOM);

        mDividerColor = ta.getColor(R.styleable.MySlidingScaleTabLayout_tl_divider_color, Color.parseColor("#ffffff"));
        mDividerWidth = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_divider_width, dp2px(0));
        mDividerPadding = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_divider_padding, dp2px(12));

        mTextUnSelectSize = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_textUnSelectSize, sp2px(14));
        // 被选中的文字大小，默认额未选中的大小一样
        mTextSelectSize = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_textSelectSize, mTextUnSelectSize);

        mTextSelectColor = ta.getColor(R.styleable.MySlidingScaleTabLayout_tl_textSelectColor, Color.parseColor("#ffffff"));
        mTextUnSelectColor = ta.getColor(R.styleable.MySlidingScaleTabLayout_tl_textUnSelectColor, Color.parseColor("#AAffffff"));
        mTextBold = ta.getInt(R.styleable.MySlidingScaleTabLayout_tl_textBold, TEXT_BOLD_NONE);
        mTextAllCaps = ta.getBoolean(R.styleable.MySlidingScaleTabLayout_tl_textAllCaps, false);

        mTabSpaceEqual = ta.getBoolean(R.styleable.MySlidingScaleTabLayout_tl_tab_space_equal, false);
        mTabWidth = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_tab_width, dp2px(-1));
        mTabPadding = ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_tab_padding, mTabSpaceEqual || mTabWidth > 0 ? dp2px(0) : dp2px(20));
        // 得到设置的上下间距和gravity
        mTabMarginTop = ta.getDimensionPixelSize(R.styleable.MySlidingScaleTabLayout_tl_tab_marginTop, 0);
        mTabMarginBottom = ta.getDimensionPixelSize(R.styleable.MySlidingScaleTabLayout_tl_tab_marginBottom, 0);

        mTabHorizontalGravity = ta.getInt(R.styleable.MySlidingScaleTabLayout_tl_tab_horizontal_gravity, CENTER);
        mTabVerticalGravity = ta.getInt(R.styleable.MySlidingScaleTabLayout_tl_tab_vertical_gravity, CENTER);

        mTabMsgMarginTop = (int) ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_tab_msg_marginTop, 0f);
        mTabMsgMarginRight = (int) ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_tab_msg_marginRight, 0f);

        mTabDotMarginTop = (int) ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_tab_dot_marginTop, 0f);
        mTabDotMarginRight = (int) ta.getDimension(R.styleable.MySlidingScaleTabLayout_tl_tab_dot_marginRight, 0f);
        mTabBackgroundId = ta.getResourceId(R.styleable.MySlidingScaleTabLayout_tl_tab_background, 0);
        openDmg = ta.getBoolean(R.styleable.MySlidingScaleTabLayout_tl_openTextDmg, false);
        ta.recycle();

        iTabScaleTransformer = new MyTabScaleTransformer(this, mTextSelectSize, mTextUnSelectSize, openDmg);
    }
...
//        Log.i("onPageScrolled", "mCurrentTab：" + mCurrentTab + " positionOffset:" + positionOffset);
            if (mCurrentPositionOffset == 0) {
                ...
                if (mPager2ScrollListener != null) {
                    mPager2ScrollListener.pageScrolled(position);
                }
            }
        }
    };

    public interface ViewPager2ScrollListener {
        void pageScrolled(int position);
    }

    private ViewPager2ScrollListener mPager2ScrollListener;

    public void setScrollListener(ViewPager2ScrollListener mPager2ScrollListener) {
        this.mPager2ScrollListener = mPager2ScrollListener;
    }
    ...
    private void initTransformer() {
        ...
        if (mTextUnSelectSize != mTextSelectSize) {
            extendTransformer = new MyExtendTransformer();
            this.mViewPager.setPageTransformer(extendTransformer);
        }
    }
...
//        iTabScaleTransformer.setNormalWidth(position, imageView.getDrawable().getIntrinsicWidth(), position == mViewPager.getCurrentItem());
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            final TextView tab_title = tabView.findViewById(R.id.tv_tab_title);
    ...
    public void setCurrentTab(int currentTab, boolean smoothScroll) {
        ...
        if (mCurrentTab != currentTab) {
            ...
            if (mViewPager != null) {
                MyPagerHelper.setCurrentItem(mViewPager, currentTab, 1);
