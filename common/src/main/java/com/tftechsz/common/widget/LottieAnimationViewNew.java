 ...
 * 多了isshow  不然使用itemnotifyrange （有延迟参数）会不显示动画问题
 */
@SuppressWarnings({"WeakerAccess"})
public class LottieAnimationViewNew extends AppCompatImageView {

    private static final String TAG = LottieAnimationViewNew.class.getSimpleName();
    ...
    public boolean flagLAV;

    @Override
    public boolean isShown() {
        if (flagLAV) {//首页修改闪烁bug问题
            return true;
        } else
    ...
    public LottieAnimationViewNew(Context context) {
        super(context);
        init(null, R.attr.lottieAnimationViewStyle);
    }

    public LottieAnimationViewNew(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, R.attr.lottieAnimationViewStyle);
    }

    public LottieAnimationViewNew(Context context, AttributeSet attrs, int defStyleAttr) {
