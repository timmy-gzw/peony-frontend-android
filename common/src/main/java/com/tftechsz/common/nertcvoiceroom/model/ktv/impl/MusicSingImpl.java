package com.tftechsz.common.nertcvoiceroom.model.ktv.impl;
...
import com.tftechsz.common.nertcvoiceroom.model.custom.CustomAttachmentType;
import com.tftechsz.common.nertcvoiceroom.model.custom.MusicControl;
import com.tftechsz.common.nertcvoiceroom.model.ktv.LyricListener;
import com.tftechsz.common.nertcvoiceroom.model.ktv.Music;
import com.tftechsz.common.nertcvoiceroom.model.ktv.MusicChangeListener;
import com.tftechsz.common.nertcvoiceroom.model.ktv.MusicOrderedItem;
...
public class MusicSingImpl extends MusicSing {
        ...
        chatRoomService.updateQueueEx(roomId, getKey(music), GsonUtils.toJson(musicOrderedItem), true).setCallback(
                new RequestCallback<Void>() {
                    ...
                    public void onSuccess(Void param) {
                        LogUtil.i(LOG_TAG, "add music success music = " + music.toString());
                        if (callback != null) {
    ...
    public void sendCustomMessage(MusicControl customMessage) {
        ...
        if (roomCallback != null) {
            VoiceRoomMessage msg = VoiceRoomMessage.createEventMessage(
                    NERTCVideoCallImpl.getMessageTextBuilder().musicEvent(customMessage.operator, customMessage.getType() == CustomAttachmentType.MUSIC_PAUSE));
        ...
        chatRoomService.pollQueue(roomId, music.getKey()).setCallback(
                new RequestCallbackWrapper<Entry<String, String>>() {
                    ...
                    public void onResult(int code, Entry<String, String> result, Throwable exception) {
                        LogUtil.i(LOG_TAG, "removeMusic musicKey = " + music.getKey() + " result code = " + code);
                        if (callback != null) {
    ...
    private void postValue(MusicChangeListener listener, boolean isInit) {
        ...
        LogUtil.i(LOG_TAG, "postValue");
        if (orderedMusics.size() > 0) {
            MusicOrderedItem topMusic = orderedMusics.get(0);
            if (equalCurrentMusic(topMusic)) {
                LogUtil.i(LOG_TAG, "postValue  music not change");
                return;
            }
            LogUtil.i(LOG_TAG, "postValue  music change");
    ...
    private List<MusicOrderedItem> getOrderMusic(List<Entry<String, String>> listParam) {
        ...
        try {
            ...
            LogUtil.e(LOG_TAG, "json parse error", e);
            return null;
        }
        return musicOrder;
    }
