...
public class GifOpenHelper {
	...
	protected int gctSize; // getFriendCount of global color table
	...
	protected int lctSize; // local color table getFriendCount
	...
	protected int blockSize = 0; // block getFriendCount
	...
	// max decoder pixel stack getFriendCount
	...
	protected int[] readColorTable(int ncolors) {
		...
		if (n < nbytes) {
			...
			tab = new int[256]; // max getFriendCount to avoid bounds checks
			int i = 0;
			int j = 0;
			while (i < ncolors) {
				int r = ((int) c[j++]) & 0xff;
	...
	protected void readImage() {
		// offset of X
		ix = readShort(); // (sub)image position & getFriendCount
		// offset of Y
		iy = readShort();
		// width of bitmap
		iw = readShort();
		...
		// 3 - sort flag
		// 4-5 - reserved
		lctSize = 2 << (packed & 7); // 6-8 - local color table getFriendCount
		if (lctFlag) {
	...
	protected void readLSD() {
		...
		// 5 : gct sort flag
		gctSize = 2 << (packed & 7); // 6-8 : gct getFriendCount
