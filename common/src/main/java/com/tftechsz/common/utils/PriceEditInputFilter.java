...
public
         ...
         *  描 述 : TODO
         */
class PriceEditInputFilter implements InputFilter {

    private final double MAX_PRICE; //最大金额
    private final DecimalFormat df = new DecimalFormat("#.##");
    private final EditText editText;
    private final int editType; //0 金币   1 红包个数
    /**
     * 小数点后的数字的位数
     */
    ...
    public PriceEditInputFilter(double max_price, EditText editText, int editType) {
...
//        dend
        ...
        if (sumText > MAX_PRICE) {
            if (editType == 0) {
                Utils.toast("单次红包金额最大" + df.format(MAX_PRICE) + "金币");
            } else if (editType == 1) {
                Utils.toast("单次最多可发" + df.format(MAX_PRICE) + "个红包");
            } else if (editType == 2) {
                Utils.toast("单次最多可兑换" + df.format(MAX_PRICE) + "个金币");
            }
            editText.setText("");
            editText.setText(df.format(MAX_PRICE));
            editText.setSelection(df.format(MAX_PRICE).length());
            return "";
        }
        //如果输入的金额小于事先规定的金额
