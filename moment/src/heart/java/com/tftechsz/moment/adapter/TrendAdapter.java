...
//        helper.setGone(R.id.iv_auth, item.getIs_self() != 1);  //是否实名
       ...
       /* new ConstraintUtil(holder.constraintLayout)
        ...
        if (null != item.getMedia()) {
            ...
            holder.setTextColor(R.id.tv_accost, item.isAccost() ? ContextCompat.getColor(getContext(), R.color.colorPrimary) : ContextCompat.getColor(getContext(), R.color.white));//私聊/搭讪
