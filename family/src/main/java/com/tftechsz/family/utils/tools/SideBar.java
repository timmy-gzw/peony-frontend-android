...
//    // 字母数组
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < b.length; i++) {
            paint.setColor(getResources().getColor(R.color.color_light_font));
            paint.setTypeface(Typeface.DEFAULT_BOLD);
            // 抗锯齿
            paint.setAntiAlias(true);
            int default_size = DensityUtils.dp2px(mContext, 12);
            paint.setTextSize(default_size);

            //选中的颜色
            if (i == choose) {
                paint.setColor(getResources().getColor(R.color.color_F8D423));
                // 设置为加粗字体
