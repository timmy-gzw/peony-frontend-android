...
//                RxBus.getDefault().post(new CommonEvent(Constants.NOTIFY_REQUEST_LOC));
        ...
        if (fragments != null && fragments.size() > 0) {
            Utils.runOnUiThread(() -> {
                for (Fragment fragment : fragments) {
                    if (fragment instanceof RecommendUserFragment) {
                        if (((RecommendUserFragment) fragment).mType == RecommendUserFragment.TYPE_NEAR) {
                            ((RecommendUserFragment) fragment).updateLocationUi(mOpenLocationType);
                            break;
                        }
                    }
                }
    ...
    public void autoRefresh() {
        if (fragments != null && fragments.get(mViewPager.getCurrentItem()) != null) {
            if (fragments.get(mViewPager.getCurrentItem()) instanceof RecommendUserFragment) {
                if (((RecommendUserFragment) fragments.get(mViewPager.getCurrentItem())).mSmartRefreshLayout != null) {
                    ((RecommendUserFragment) fragments.get(mViewPager.getCurrentItem())).mSmartRefreshLayout.autoRefresh();
                    ((RecommendUserFragment) fragments.get(mViewPager.getCurrentItem())).mLayoutManager.smoothScrollToPosition(((RecommendUserFragment) fragments.get(mViewPager.getCurrentItem())).mRvUser, new RecyclerView.State(), 0);
                }
            }
        }
    }
